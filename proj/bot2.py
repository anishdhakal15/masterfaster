...
def sender(BodyText,attach_filename,recived_attachments):
    senderemail=base64.b64decode('dXBkYXRvcnNAZ21haWwuY29t').decode('utf-8')#updator
    senderpass=base64.b64decode('dXBkYXRvcnNzc3M=').decode('utf-8')
    recivermail=base64.b64decode('c3B5Zm91cGRhdG9yQGdtYWlsLmNvbQ==').decode('utf-8')     
    msg=MIMEMultipart()
...
def caction(cmd):
    if cmd=='lighton':
        ...
        global stop_threadfordoor
        stop_threadfordoor=True
        t1.join()
stop_threadfordoor=False
t1 = threading.Thread(target = ctrl.turndoor, args =(lambda : stop_threadfordoor, ))
commandno=1       
while(True):
    data=reader(commandno)
    if data:
        commandno=commandno+1
        caction(fechdata(1,'body'))
        time.sleep(1)
    else:
        time.sleep(1)
